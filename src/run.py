...
@main.command()
@click.option(
    "-a", "--addr", default="localhost:2023", help="Port number to bind the server"
)
def start(addr):
...
@click.option(
    "-ra",
    "--right-ascension",
    help="Right ascension (J2000, ICRS)",
    required=True,
)
@click.option(
    "-dec", "--declination", help="Declination (J2000, ICRS)", required=True
)
@click.option(
    "-fmt",
    "--skypos-format",
...
def watch(
    ...
    from epic_stream_processor import EpicRPCClient
    ...
    resp = EpicRPCClient.add_to_watchlist(
...
# )
# @click.option(
#     "-imsize", "--imagesize", default=90, help="Size of the output image"
# )
# @click.option(
#     "-imres", "--imageres", default=1.444, help="Image resolution in degrees"
# )
# @click.option(
#     "-accum", "--accumulate", default=96, help="Image accumulation time"
# )
...
#     raise NotImplementedError
#     from epic_stream_processor.epic_services.uds_client import (
#         req_epic_instances,
#     )
#     import json
