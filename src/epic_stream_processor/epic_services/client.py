...
class EpicRPCClient:
    def __init__(self) -> None:
        ...
        self._stub = epic_image_pb2_grpc.epic_post_processStub(self._channel)
    ...
    def get_dummy_data(self) -> Iterator[epic_image]:  # type: ignore[no-any-unimported]
        with fits.open(
            res_path(example_data, "EPIC_1661990950.000000_73.487MHz.fits")
        ) as hdu:
            for i in range(1):
    ...
    def send_data(self, header_arr: List[str], data: NDArrayNum_t) -> None:
        return self._stub.filter_and_save_chunk(self.chunk_data(header_arr, data))
